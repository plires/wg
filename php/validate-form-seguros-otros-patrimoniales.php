<?php
  ...
  $db = new RepositorioSQL();
	...
	if($datos['success'] == 1 && $datos['score'] >= 0.5){
		...
		if (emptyInput( isset($_POST['origin'])) ) {
	    $_POST['origin'] = 'Seguros Patrimoniales';
	  }
	  ...
	  if ( count($errors) == 0 ) {
		  ...
		  if ($sendEmails) {
		    ...
		    header("Location: " . $url[0] ."?msg_seguros=". urlencode($msg_seguros) . "#form_seguros" );

		  } else {

		    header("Location: " . $url[0] . "?" . http_build_query( array( "errors[send]" => 'La consulta fue registrada, sin embargo no puderon enviarse los correos de agradecimiento. Si tiene alguna duda vuelva a contactarse por favor' ), $email ) . "#form_seguros" );
  	...
  	header("Location: " . $url[0] . "?" . http_build_query( array( "errors[recaptcha]" => 'Error al validar el recaptcha... intente nuevamente por favor' ), $email ) . "#form_seguros" );
